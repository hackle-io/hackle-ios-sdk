...
class DefaultHackleCoreSpecs: QuickSpec {
    ...
    static let sharedFlowFactory: MockInAppMessageEligibilityFlowFactory = {
        let factory = MockInAppMessageEligibilityFlowFactory()
        EvaluationContext.shared.register(factory)
        return factory
    }()
