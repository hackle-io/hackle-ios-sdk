...
class LruWorkspaceEvaluationCacheSpecs: QuickSpec {
    override class func spec() {
        ...
        it("put은 전체 스냅샷을 오래된 순으로 반환한다") {
            _ = sut.put(context: record(id: "1"))
            let snapshots = sut.put(context: record(id: "2"))
        ...
        it("capacity 초과 시 가장 오래된 record를 evict한다") {
            _ = sut.put(context: record(id: "1"))
            _ = sut.put(context: record(id: "2"))
            _ = sut.put(context: record(id: "3"))
            let snapshots = sut.put(context: record(id: "4"))
        ...
        it("같은 key를 다시 put하면 순서가 최신으로 갱신된다") {
            _ = sut.put(context: record(id: "1"))
            _ = sut.put(context: record(id: "2"))
            _ = sut.put(context: record(id: "1")) // 재삽입
            let snapshots = sut.put(context: record(id: "3"))
        ...
        it("restore는 기존 항목을 비우고 뒤에서 capacity개만 복원한다") {
            _ = sut.put(context: record(id: "old"))

            sut.restore(contexts: [record(id: "1"), record(id: "2"), record(id: "3"), record(id: "4")])
