...
class ExperimentFlowEvaluatorSpecs: QuickSpec {
    override class func spec() {
        ...
        var nextFlow: ExperimentLocalEvaluationFlow!
        var context: EvaluatorContext!

        beforeEach {
            nextFlow = ExperimentLocalEvaluationFlow.end()
            context = Evaluators.context()
        }

        describe("OverrideExperimentLocalFlowEvaluator") {

            var overrideResolver: MockOverrideResolver!
            var sut: OverrideExperimentLocalFlowEvaluator!

            beforeEach {
                overrideResolver = MockOverrideResolver()
                sut = OverrideExperimentLocalFlowEvaluator(overrideResolver: overrideResolver)
            }
        ...
        describe("DraftExperimentLocalFlowEvaluator") {
            it("DRAFT상태면 기본그룹으로 평가한다") {
                ...
                // when
                let actual = try DraftExperimentLocalFlowEvaluator().evaluate(request: request, context: context, nextFlow: nextFlow)
        ...
        describe("PausedExperimentLocalFlowEvaluator") {
            ...
            it("AB 테스트가 PAUSED 상태면 기본그룹, EXPERIMENT_PAUSED으로 평가한다") {
                ...
                // when
                let actual = try PausedExperimentLocalFlowEvaluator().evaluate(request: request, context: context, nextFlow: nextFlow)
        ...
        describe("CompletedExperimentLocalFlowEvaluator") {
            ...
            it("COMPLETED 상태면 위너 그룹 평가한다") {
                ...
                // when
                let actual = try CompletedExperimentLocalFlowEvaluator().evaluate(request: request, context: context, nextFlow: nextFlow)
            ...
            it("COMPLETED 상태지만 winner variation 이 없으면 에러") {
                ...
                // when
                let actual = expect(try CompletedExperimentLocalFlowEvaluator().evaluate(request: request, context: context, nextFlow: nextFlow))
        ...
        describe("TargetExperimentLocalFlowEvaluator") {

            var experimentTargetDeterminer: MockExperimentTargetDeterminer!
            var sut: TargetExperimentLocalFlowEvaluator!
            beforeEach {
                experimentTargetDeterminer = MockExperimentTargetDeterminer()
                sut = TargetExperimentLocalFlowEvaluator(experimentTargetDeterminer: experimentTargetDeterminer)
            }
        ...
        describe("TrafficAllocateExperimentLocalFlowEvaluator") {

            var actionResolver: MockActionResolver!
            var sut: TrafficAllocateExperimentLocalFlowEvaluator!

            beforeEach {
                actionResolver = MockActionResolver()
                sut = TrafficAllocateExperimentLocalFlowEvaluator(actionResolver: actionResolver)
            }
        ...
        describe("TargetRuleExperimentLocalFlowEvaluator") {

            var targetRuleDeterminer: MockExperimentTargetRuleDeterminer!
            var actionResolver: MockActionResolver!
            var sut: TargetRuleExperimentLocalFlowEvaluator!

            beforeEach {
                targetRuleDeterminer = MockExperimentTargetRuleDeterminer()
                actionResolver = MockActionResolver()
                sut = TargetRuleExperimentLocalFlowEvaluator(targetRuleDeterminer: targetRuleDeterminer, actionResolver: actionResolver)
            }
        ...
        describe("DefaultRuleExperimentLocalFlowEvaluator") {

            var actionResolver: MockActionResolver!
            var sut: DefaultRuleExperimentLocalFlowEvaluator!

            beforeEach {
                actionResolver = MockActionResolver()
                sut = DefaultRuleExperimentLocalFlowEvaluator(actionResolver: actionResolver)
            }
        ...
        describe("ContainerExperimentLocalFlowEvaluator") {

            var containerResolver: MockContainerResolver!
            var sut: ContainerExperimentLocalFlowEvaluator!

            beforeEach {
                containerResolver = MockContainerResolver()
                sut = ContainerExperimentLocalFlowEvaluator(containerResolver: containerResolver)
            }
        ...
        describe("IdentifierExperimentLocalFlowEvaluator") {

            var sut: IdentifierExperimentLocalFlowEvaluator!

            beforeEach {
                sut = IdentifierExperimentLocalFlowEvaluator()
            }
