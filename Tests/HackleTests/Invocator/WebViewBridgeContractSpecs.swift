...
class WebViewBridgeContractSpecs: QuickSpec {
    override class func spec() {
        ...
        it("messageId가 붙어도 기존 invocator가 그대로 파싱한다") {
        ...
        it("mutation 메시지는 core를 호출하고 완료 후 resolve를 발송한다") {
            MainActor.assumeIsolated {
                ...
                expect(script).to(beginWith("window._hackleBridge && window._hackleBridge.resolveMessage(\"\(messageId)\", \"{"))
        ...
        it("track 메시지도 core를 호출하고 완료 후 resolve를 발송한다") {
            MainActor.assumeIsolated {
                ...
                expect(webView.evaluatedScripts.count).toEventually(equal(1), timeout: .seconds(3))
                expect(webView.evaluatedScripts[0])
                    .to(beginWith("window._hackleBridge && window._hackleBridge.resolveMessage(\"\(messageId)\", \"{"))
            }
        }
    }
}
